...
<html>
  ...
  <!-- Facebook and Twitter integration -->
    ...
    <?php if (isset($_GET["formato"])){ ?>
            <script type="text/javascript">
                window.onload = function(){ window.alert("El formato del archivo enviado no es de una imagen aceptable."); };
            </script>
    <?php } ?>
			...
			<div class="container-fluid">
				<div class="row">
					<div class="col-12 mt-5 bg-white rounded">
						...
						<div class="row mb-3">
							<h3 class="text-dark font-weight-bold mx-3 my-auto col-auto">Anuncios</h3>
		                    ...
		                    <a class="btn btn-success my-auto ml-auto mr-3 text-white" href="FormAds.php">
		                        Agregar registro
		                    </a>
	                    </div>

						 <?php
		                    $re2 = $mysqli->query("SELECT * FROM `anuncios`");
		                    ...
		                    if(isset($_GET['buscar'])){
		                        $re = $mysqli->query("SELECT * FROM anuncios WHERE ` descripcion_anuncio`  LIKE '%".$_GET['buscar']."%' ORDER BY `idAnuncios` ASC LIMIT $empieza, $cantidad_resultados");
		                    } else {
		                        $re = $mysqli->query("SELECT * FROM anuncios ORDER BY `idAnuncios` ASC LIMIT $empieza, $cantidad_resultados");
		                    }
						...
						<table class="table table-striped table-bordered table-sm table-hover">
							<thead class="thead-dark bg-dark text-white">
								...
								<th class="text-center col-auto">Admin</th>
								<th class="text-center col-auto">Titulo</th>
								<th class="text-center col-auto">Fecha</th>
								<th class="text-center col-auto">Ver descripción</th>
								<th class="text-center col-auto">Ver Imagen</th>
		                ...
		                ?>  
							<tr>
		                        <td class="text-center"><?php echo $f['idAnuncios']; ?></td>
		                        <td class="text-center"><?php echo $f['Administrador_idAdministrador']; ?></td>
		                        <td class="text-center"><?php echo $f['titulo_anuncio']; ?></td>
		                        <td class="text-center"><?php echo $f['fecha_anuncio']; ?></td>
		                        <td class="text-center">
		                            <button class="btn btn-warning btn-sm" name="id" value="<?php echo $f['idAnuncios']; ?>" onclick="window.alert('<?php echo $f['descripcion_anuncio']; ?>');">
		                                <i class="icon-doc-text" aria-hidden="true"></i>
		                            </button>
		                        </td>
		                        <td class="text-center">
		                            <a class="btn btn-warning btn-sm" href="images/<?php echo $f['imagen_anuncio']; ?>" target=”_blank”>
		                                <i class="icon-sticky-note" aria-hidden="true"></i>
		                            </a>
		                        </td>
		                        <td class="text-center">
		                        	<a href="FormAds.php?id=<?php echo $f['idAnuncios']; ?>" class="btn btn-info btn-sm">
		                                <i class="icon-edit" aria-hidden="true"></i>
		                            </a>
		                        </td>
		                        <td class="text-center">
		                            <form action="php/DeleteAds.php" method="POST">
		                                <button type="submit" class="btn btn-danger btn-sm" name="id" value="<?php echo $f['idAnuncios']; ?>">
