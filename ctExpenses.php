			...
			<div class="container-fluid">
				<div class="row">
					<div class="col-12 mt-5 bg-white rounded">
						...
						<div class="row mb-3">
							<h3 class="text-dark font-weight-bold mx-3 my-auto col-auto">Gastos</h3>
		                    ...
		                    <a class="btn btn-success my-auto ml-auto mr-3 text-white" href="FormExpenses.php">
		                        Agregar registro
		                    </a>
	                    </div>

						 <?php
		                    $re2 = $mysqli->query("SELECT * FROM `gastos`");
		                    ...
		                    if(isset($_GET['buscar'])){
		                        $re = $mysqli->query("SELECT * FROM gastos WHERE `Administrador_idAdministrador`  LIKE '%".$_GET['buscar']."%' ORDER BY `idGastos` ASC LIMIT $empieza, $cantidad_resultados");
		                    } else {
		                        $re = $mysqli->query("SELECT * FROM gastos ORDER BY `idGastos` ASC LIMIT $empieza, $cantidad_resultados");
		                    }
		                ...
		                ?>  
							<tr>
		                        <td class="text-center"><?php echo $f['idGastos']; ?></td>
		                        <td class="text-center"><?php echo $f['Administrador_idAdministrador']; ?></td>
		                        <td class="text-center"><?php echo $f['cantidad_gasto']; ?></td>
		                        <td class="text-center"><?php echo $f['fecha_gasto']; ?></td>
		                        <td>
		                        	<a href="/FormExpenses.php?id=<?php echo $f['idGastos']; ?>" class="btn btn-info btn-sm">
		                                <i class="icon-edit" aria-hidden="true"></i>
		                            </a>
		                        </td>
		                        <td class="text-center">
		                            <form action="php/DeleteExpense.php" method="POST">
		                                <button type="submit" class="btn btn-danger btn-sm" name="id" value="<?php echo $f['idGastos']; ?>">
