			...
			<div class="container-fluid">
				<div class="row">
					<div class="col-12 mt-5 bg-white rounded">
						...
						<div class="row mb-3">
							<h3 class="text-dark font-weight-bold mx-3 my-auto col-auto">Ventas</h3>
		                    ...
		                    <a class="btn btn-success my-auto ml-auto mr-3 text-white" href="FormSales.php">
		                        Agregar registro
		                    </a>
	                    </div>

						 <?php
		                    $re2 = $mysqli->query("SELECT *, DATE_FORMAT(fecha_venta, '%d-%m-%Y')AS fecha_venta_F FROM `ventas`");
		                    ...
		                    if(isset($_GET['buscar'])){
		                        $re = $mysqli->query("SELECT *, DATE_FORMAT(fecha_venta, '%d-%m-%Y') AS fecha_venta_F FROM ventas WHERE `Productos_idProductos`  LIKE '%".$_GET['buscar']."%' ORDER BY `idVentas` ASC LIMIT $empieza, $cantidad_resultados");
		                    } else {
		                        $re = $mysqli->query("SELECT *, DATE_FORMAT(fecha_venta, '%d-%m-%Y') AS fecha_venta_F FROM ventas ORDER BY `idVentas` ASC LIMIT $empieza, $cantidad_resultados");
		                    }
						...
						<table class="table table-striped table-bordered table-sm table-hover">
							<thead class="thead-dark bg-dark text-white">
								...
								<th class="text-center col-auto">Total</th>
								<th class="text-center col-auto">Folio</th>
		                ...
		                ?>  
							<tr>
		                        <td class="text-center"><?php echo $f['idVentas']; ?></td>
		                        <td class="text-center"><?php echo $f['Administrador_idAdministrador']; ?></td>
		                        <td class="text-center"><?php echo $f['total_venta']; ?></td>
		                        <td class="text-center"><?php echo $f['folio_venta']; ?></td>
		                        <td class="text-center"><?php echo $f['fecha_venta_F']; ?></td>
		                        <td class="text-center">
		                            <a class="btn btn-warning btn-sm" href=php/recibo.php?id=<?php echo $f['idVentas']; ?> target=”_blank”>
		                                <i class="icon-sticky-note" aria-hidden="true"></i>
		                            </a>
		                        </td>
		                        <td class="text-center">
		                            <form action="php/DeleteSale.php" method="POST">
		                                <button type="submit" class="btn btn-danger btn-sm" name="id" value="<?php echo $f['idVentas']; ?>">
