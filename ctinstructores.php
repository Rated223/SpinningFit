  ...
  <!-- Facebook and Twitter integration -->
	...
	<?php if (isset($_GET["repetidos"])){ ?>
            <script type="text/javascript">
                window.onload = function(){ window.alert("Los datos de usuario y correo ya estan registrados en otras cuentas, por favor elija otros."); };
            </script>
    <?php } ?>
    <?php if (isset($_GET["errorusuario"])){ ?>
            <script type="text/javascript">
                window.onload = function(){ window.alert("Este nombre de usuario ya existe, por favor escoja otro."); };
            </script>
    <?php } ?>
    <?php if (isset($_GET["errorcorreo"])){ ?>
            <script type="text/javascript">
                window.onload = function(){ window.alert("Este correo ya ha sido ingresado, por favor ingrese otro."); };
            </script>
    <?php } ?>
    <?php if (isset($_GET["errorcontra"])){ ?>
            <script type="text/javascript">
                window.onload = function(){ window.alert("Las contraseñas no coinciden."); };
            </script>
    <?php } ?>
			...
			<div class="container-fluid">
				<div class="row">
					<div class="col-12 mt-5 bg-white rounded">
						...
						<div class="row mb-3">
							<h3 class="text-dark font-weight-bold mx-3 my-auto col-auto">Instructores</h3>
		                    ...
		                    <a class="btn btn-success my-auto ml-auto mr-3 text-white" href="registration.php">
		                        Agregar registro
		                    </a>
	                    </div>
						 ...
						 <?php
		                    ...
		                    if(isset($_GET['buscar'])){
		                        $re = $mysqli->query("SELECT * FROM instructor WHERE ` usuario_instructor`  LIKE '%".$_GET['buscar']."%' ORDER BY `idinstructor` ASC LIMIT $empieza, $cantidad_resultados");
		                    } else {
		                        $re = $mysqli->query("SELECT * FROM instructor ORDER BY `idinstructor` ASC LIMIT $empieza, $cantidad_resultados");
		                    }
						...
						<table class="table table-striped table-bordered table-sm table-hover">
							<thead class="thead-dark bg-dark text-white">
								...
								<th class="text-center col-auto">Dirección</th>
								<th class="text-center col-auto">Correo</th>
								<th class="text-center col-auto">telefono</th>
								<th class="text-center col-auto">Sueldo</th>
		                ...
		                ?>  
							<tr>
		                        <td class="text-center"><?php echo $f['idinstructor']; ?></td>
		                        <td class="text-center"><?php echo $f['usuario_instructor']; ?></td>
		                        <td class="text-center"><?php echo $f['nombre_instructor']; ?></td>
		                        <td class="text-center"><?php echo $f['apellido_instructor']; ?></td>
		                        <td class="text-center"><?php echo $f['direccion_instructor']; ?></td>
		                        <td class="text-center"><?php echo $f['correo_instructor']; ?></td>
		                        <td class="text-center"><?php echo $f['telefono_instructor']; ?></td>
		                        <td class="text-center"><?php echo $f['sueldo_instructor']; ?></td>
		                        <td class="text-center">
		                        	<form action="EditInstructor.php" method="POST">
			                            <button type="submit" class="btn btn-info btn-sm" name="id" value="<?php echo $f['idinstructor']; ?>">
			                                <i class="icon-edit" aria-hidden="true"></i>
			                            </button>
		                            </form>
		                        </td>
		                        <td class="text-center">
		                            <form action="php/DeleteInstructor.php" method="POST">
		                                <button type="submit" class="btn btn-danger btn-sm" name="id" value="<?php echo $f['idinstructor']; ?>">
