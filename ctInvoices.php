			...
			<div class="container-fluid">
				<div class="row">
					<div class="col-12 mt-5 bg-white rounded">
						...
						<div class="row mb-3">
							<h3 class="text-dark font-weight-bold mx-3 my-auto col-auto">facturas</h3>
		                    ...
		                    <a class="btn btn-success my-auto ml-auto mr-3 text-white" href="FormInvoices.php">
		                        Agregar registro
		                    </a>
	                    </div>

						 <?php
		                    $re2 = $mysqli->query("SELECT * FROM `facturas`");
		                    ...
		                    if(isset($_GET['buscar'])){
		                        $re = $mysqli->query("SELECT * FROM facturas WHERE `Alumnos_idAlumnos`  LIKE '%".$_GET['buscar']."%' ORDER BY `idFacturas` ASC LIMIT $empieza, $cantidad_resultados");
		                    } else {
		                        $re = $mysqli->query("SELECT * FROM facturas ORDER BY `idFacturas` ASC LIMIT $empieza, $cantidad_resultados");
		                    }
						...
						<table class="table table-striped table-bordered table-sm table-hover">
							<thead class="thead-dark bg-dark text-white">
								...
								<th class="text-center col-auto">Cliente</th>
								<th class="text-center col-auto">Fecha</th>
								<th class="text-center col-auto">Total</th>
								<th class="text-center col-auto">Ver documento</th>
		                ...
		                ?>  
							<tr>
		                        <td class="text-center"><?php echo $f['idFacturas']; ?></td>
		                        <td class="text-center"><?php echo $f['Alumnos_idAlumnos']; ?></td>
		                        <td class="text-center"><?php echo $f['fecha_factura']; ?></td>
		                        <td class="text-center"><?php echo $f['total_factura']; ?></td>
								<td class="text-center">
		                        	<form action="" method="POST">
			                            <button type="submit" class="btn btn-warning btn-sm" name="id" value="<?php echo $f['idFacturas']; ?>">
			                                <i class="icon-edit" aria-hidden="true"></i>
			                            </button>
		                            </form>
		                        </td>
		                        <td class="text-center">
		                        	<a href="/FormInvoices.php?id=<?php echo $f['idFacturas']; ?>" class="btn btn-info btn-sm">
		                                <i class="icon-edit" aria-hidden="true"></i>
		                            </a>
		                        </td>
		                        <td class="text-center">
		                            <form action="php/DeleteInvoice.php" method="POST">
		                                <button type="submit" class="btn btn-danger btn-sm" name="id" value="<?php echo $f['idFacturas']; ?>">
