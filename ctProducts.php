...
<html>
  ...
  <!-- Facebook and Twitter integration -->
    ...
    <?php if (isset($_GET["errorcodigo"])){ ?>
            <script type="text/javascript">
                window.onload = function(){ window.alert("Ya existe un producto con este codigo."); };
            </script>
    <?php } ?>
			...
			<div class="container-fluid">
				<div class="row">
					<div class="col-12 mt-5 bg-white rounded">
						...
						<div class="row mb-3">
							<h3 class="text-dark font-weight-bold mx-3 my-auto col-auto">Productos</h3>
		                    ...
		                    <a class="btn btn-success my-auto ml-auto mr-3 text-white" href="FormProducts.php">
		                        Agregar registro
		                    </a>
	                    </div>

						 <?php
		                    $re2 = $mysqli->query("SELECT * FROM `productos`");
		                    ...
		                    if(isset($_GET['buscar'])){
		                        $re = $mysqli->query("SELECT * FROM productos WHERE `nombre_producto`  LIKE '%".$_GET['buscar']."%' ORDER BY `idProductos` ASC LIMIT $empieza, $cantidad_resultados");
		                    } else {
		                        $re = $mysqli->query("SELECT * FROM productos ORDER BY `idProductos` ASC LIMIT $empieza, $cantidad_resultados");
		                    }
						...
						<table class="table table-striped table-bordered table-sm table-hover">
							<thead class="thead-dark bg-dark text-white">
								...
								<th class="text-center col-auto">Codigo</th>
								<th class="text-center col-auto">Precio</th>
		                ...
		                ?>  
							<tr>
		                        <td class="text-center"><?php echo $f['idProductos']; ?></td>
		                        <td class="text-center"><?php echo $f['Administrador_idAdministrador']; ?></td>
		                        <td class="text-center"><?php echo $f['nombre_producto']; ?></td>
		                        <td class="text-center"><?php echo $f['codigo_producto']; ?></td>
		                        <td class="text-center"><?php echo $f['precio_producto']; ?></td>
		                        <td class="text-center"><?php echo $f['cantidad_producto']; ?></td>
		                        <td class="text-center">
		                        	<form action="FormProducts.php" method="POST">
			                            <button type="submit" class="btn btn-info btn-sm" name="id" value="<?php echo $f['idProductos']; ?>">
		                        ...
		                        <td class="text-center">
		                            <form action="php/DeleteProduct.php" method="POST">
		                                <button type="submit" class="btn btn-danger btn-sm" name="id" value="<?php echo $f['idProductos']; ?>">
